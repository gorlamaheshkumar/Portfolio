...
var NODE_VERT = `
  ...
  void main(){
    ...
    vDepth = -mv.z;
    gl_PointSize = (2.4 + aRnd*3.0) * (260.0 / -mv.z);
...
var LINE_FRAG = `
  ...
  void main(){
    ...
    gl_FragColor = vec4(uCol, vAlpha * (1.0 - fog) * uReveal);
  }
`;
var PKT_VERT = `
...
var nodeMat = new THREE.ShaderMaterial({
  ...
  uniforms: { uTime:{value:0}, uMouse:{value:new THREE.Vector2()}, uReveal:{value:REDUCED?1:0} },
...
var lineMat = new THREE.ShaderMaterial({
  ...
  uniforms: { uCol:{ value:new THREE.Vector3(0.0,0.94,1.0) }, uReveal:{value:REDUCED?1:0} },
  transparent:true, depthWrite:false, blending:THREE.AdditiveBlending
});
...
var pktMat = new THREE.ShaderMaterial({
  ...
  uniforms: { uScale:{ value:3.2 }, uReveal:{value:REDUCED?1:0} },
  transparent:true, depthWrite:false, blending:THREE.AdditiveBlending
});
...
if (!TOUCH && !REDUCED){
  document.querySelectorAll('[data-mag]').forEach(function(el){
    el.style.transition = 'transform .45s cubic-bezier(.34,1.56,.64,1), border-color .3s, background .3s, box-shadow .3s, color .3s';
    el.addEventListener('mousemove', function(e){
      var r = el.getBoundingClientRect();
      var mx = (e.clientX - (r.left + r.width/2)) / (r.width/2);
      var my = (e.clientY - (r.top + r.height/2)) / (r.height/2);
      el.style.transform = 'translate('+(mx*9)+'px,'+(my*9)+'px)';
    });
...
document.querySelectorAll('[data-copy]').forEach(function(btn){
  btn.addEventListener('click', function(){
    ...
    if (navigator.clipboard && navigator.clipboard.writeText){
      navigator.clipboard.writeText(val).then(function(){ popToast('COPIED ● 200 OK'); }, function(){ popToast('COPIED ● ' + val); });
    } else { popToast('COPIED ● ' + val); }
  });
});
...
(function meridLoop(){
  ...
  if (h !== lastMerid){ meridianFill.style.height = (h/10) + '%'; lastMerid = h; }
